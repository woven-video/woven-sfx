...
const catalog=JSON.parse(process.argv[1]);const lib=process.argv[2];const cdn=(process.argv[3]||'').replace(/\/+$/,'');
function resolveUrl(url){return url.startsWith('/')?cdn+url:url;}
